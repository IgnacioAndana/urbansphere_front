...
function abrirGestion(tipo: Pestana, item: SolicitudInteres | SolicitudContacto) {
  ...
  gestionEstado.value = item.estado ?? 'pendiente'
          ...
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
              ...
              <tr v-else-if="listaInteres.length === 0">
                <td colspan="6" class="p-8 text-center text-slate-400">No hay solicitudes de interés.</td>
              </tr>
              <tr v-for="s in listaInteres" :key="s.id" class="hover:bg-slate-50/80">
                <td class="p-4 text-slate-600 whitespace-nowrap">{{ s.creadoEn ?? '—' }}</td>
                <td class="p-4">
                  <RouterLink
          ...
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
              ...
              <tr v-else-if="listaContacto.length === 0">
                <td colspan="6" class="p-8 text-center text-slate-400">No hay solicitudes de contacto.</td>
              </tr>
              <tr v-for="s in listaContacto" :key="s.id" class="hover:bg-slate-50/80">
                <td class="p-4 text-slate-600 whitespace-nowrap align-top">{{ s.creadoEn ?? '—' }}</td>
